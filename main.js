...
4. 1エントリは短く、**最大5秒以内**のフレーズで区切る（テンポ重視）
5. 句読点（、。！？）は一切入れない（代わりにスペース等で調整）
6. 聞き取れない部分は（聞き取り不明）と表記する（推測で埋めない）
7. 改行は単なる文字数ではなく、**意味のまとまり（文節）**で行う
8. 「えっと」「あの」「まあ」「その」などのフィラー（言い淀み）は全て削除し、意味の通る整った文章にする
9. 固有名詞・人名・地名は文脈から正しく判別する
10. 数字は可能な限り **算用数字（1, 10, 100等）** に変換する（「一回」→「1回」）
11. SRTの番号は1から連番
